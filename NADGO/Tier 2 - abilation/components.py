...
# --- Ablation helpers (no-op / vendor-like) ---

class NoOpPadder:
    """Identity padder: returns the circuit unchanged."""
    def pad(self, circuit, **kwargs):
        return circuit

class NoJitterScheduler:
    """Scheduler with zero jitter; preserves nominal cadence."""
    def __init__(self, interval_ms: float):
        self.interval_ms = float(interval_ms)
    def propose_offset_ms(self, **_):
        return 0.0  # no jitter

class VendorRouter:
    """
    Simple vendor-like router: pick backend with the shortest queue tail
    (or round-robin if you prefer). Matches NADGO's Router interface.
    """
    def __init__(self, backends):
        self.backends = list(backends)
        self._rr = 0
    def select_backend(self, segment, queues, **_):
        # shortest-tail heuristic (cheap and deterministic)
        return min(self.backends, key=lambda h: queues[h].tail_time_ms)
        # OR: round-robin
