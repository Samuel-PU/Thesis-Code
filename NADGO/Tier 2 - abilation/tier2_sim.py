...
def parse_args() -> argparse.Namespace:
    ...
    # Ablation pipeline
    p.add_argument("--pipeline", choices=["vendor", "pad-only", "jitter-only", "full"],
                   default="full",
                   help="Ablation pipeline: vendor | pad-only | jitter-only | full (default: full)")
...
def main():
    ...
    # Detect if simulator supports emul_config / pipeline
    sim_accepts_emul = "emul_config" in inspect.signature(NADGOSimulator.__init__).parameters
    sim_accepts_pipeline = "pipeline" in inspect.signature(NADGOSimulator.__init__).parameters
    ...
    for n in args.qubits:
        for attack in args.attacks:
            for seed in seeds:
                ...
                if sim_accepts_pipeline:
                    sim_kwargs["pipeline"] = args.pipeline
                ...
                # tag output rows with pipeline
                ep_row = asdict(ep_result)
                ep_row["pipeline"] = args.pipeline
                all_episodes.append(ep_row)

                for sr in seg_results:
                    r = asdict(sr)
                    r["pipeline"] = args.pipeline
    ...
    thr_used_path = os.path.join(args.out, "thresholds_used.json")
    ...
    # Thresholds provenance for plotting overlays
    thr_used = {
        "budget": args.budget,
        "kill": args.kill,
        "monitor_beta": args.monitor_beta,
    ...
    save_json(thr_used_path, thr_used)
    ...
    print(f"  Thresholds used -> {thr_used_path}")
