...
class NADGOSimulator:
    def __init__(self, n_qubits: int, delta_budget: float = 0.04, delta_kill: float = 0.07,
                 ...
                 # >>> NEW: finite-sample estimator knobs for the monitor
                 monitor_alpha: float = 1e-3,
                 monitor_qmin: float = 1e-3,
                 monitor_eps_sync: float = 0.0):
        """
        NADGO Tier-1 simulator harness.

        Args (selected):
            delta_budget, delta_kill: thresholds in bits.
            monitor_beta: additive β term (bits) in the estimator.
            monitor_alpha: per-interval failure probability α for ε_est (default 1e-3).
            monitor_qmin: floor for q_ref bins to satisfy A4 (default 1e-3).
            monitor_eps_sync: slack ε_sync for timestamp/quantisation mismatch (bits).
        """
        ...
        # >>> NEW: store & pass estimator parameters into the LeakageMonitor
        self.monitor_beta = float(monitor_beta)
        self.monitor_alpha = float(monitor_alpha)
        self.monitor_qmin = float(monitor_qmin)
        self.monitor_eps_sync = float(monitor_eps_sync)

        self.monitor = LeakageMonitor(
            self.delta_budget,
            self.delta_kill,
            beta=self.monitor_beta,
            kill_strikes=kill_strikes,
            warn_cooldown=warn_cooldown,
            alpha=self.monitor_alpha,     # <<< NEW
            q_min=self.monitor_qmin,      # <<< NEW
            eps_sync=self.monitor_eps_sync  # <<< NEW
        )
